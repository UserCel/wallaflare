...
async function main() {
  ...
  const capacitorHtml = rawHtml
    ...
    .replace('<head>', `<head>\n  <script>window.IS_CAPACITOR_APP = true; window.WF_BUILD_VERSION = "${otaVersion}"; window.WF_NATIVE_VERSION = "1.0.0"; document.documentElement.classList.add("is-capacitor-app"); (function(){var el=document.documentElement;el.style.setProperty("--toolbar-opacity","0");var h=48;try{if(window.AndroidNative&&typeof window.AndroidNative.getStatusBarHeight==='function'){var v=window.AndroidNative.getStatusBarHeight();if(v>0)h=v;}}catch(e){}el.style.setProperty("--app-safe-top",h+"px");el.style.setProperty("--status-bar-height",h+"px");requestAnimationFrame(function(){requestAnimationFrame(function(){el.style.setProperty("--toolbar-opacity","1");});});})();</script>`);
