...
const TEST_URLS: TestCase[] = [
  // Fiction & Web Serials (Top RoyalRoad Serials)
  {
    category: 'Fiction',
  ...
  {
    category: 'Fiction',
    name: 'Hundred Reigns (Void Herald)',
  ...
  // Tech & Engineering Deep Dives
  {
    category: 'Deep Dive',
    name: 'Ciechanowski (GPS Deep Dive)',
    url: 'https://ciechanow.ski/gps/'
  },
  {
    category: 'Tech Blog',
    name: 'Julia Evans (Running SQLite)',
    url: 'https://jvns.ca/blog/2026/07/17/learning-about-running-sqlite/'
  },
  {
    category: 'Tech Blog',
    name: 'Dan Luu (Keyboard Latency)',
    url: 'https://danluu.com/keyboard-latency/'
  },
  {
    category: 'Tech Blog',
    name: 'Simon Willison Web Log',
...
async function runTests() {
  ...
  for (const test of TEST_URLS) {
    ...
    try {
      ...
      const author = (article.byline || 'Unknown').substring(0, 24);
      ...
      console.log(
        ...
        test.category.padEnd(14),
        test.name.padEnd(30),
        (err.message || 'unknown').substring(0, 24),
        '-'.padStart(7),
        '-'.padStart(4),
        String(duration).padStart(5)
      );
